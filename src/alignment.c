...
void mx_set_field_lens(t_list *entry_names, t_ls *ls, t_stat *p_stat) {
    ...
    while (node) {
        path = mx_prepare_path(ls->curr_dir_name, node->data);
        stat(path, p_stat);
        lnk_l = mx_intlen(mx_nlink(p_stat));
        usr_l = mx_strlen(mx_get_pw_name(p_stat));
        grp_l = mx_strlen(mx_get_gr_name(p_stat));
        size_l = mx_intlen(mx_get_file_size(p_stat));
        hour_year_l = mx_strlen(mx_get_hour_or_year(p_stat, ls));
